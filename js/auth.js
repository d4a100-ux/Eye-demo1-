...
function navGroups() {
  const ALL = {
    inicio:  { icon:'ti-home-2',          label:'Início'       },
    conv:    { icon:'ti-message-circle',  label:'Conversas'    },
    crm:     { icon:'ti-layout-kanban',   label:'CRM'          },
    tarefas: { icon:'ti-circle-check',    label:'Tarefas'      },
    agenda:  { icon:'ti-calendar',        label:'Agenda'       },
    cal:     { icon:'ti-calendar-month',  label:'Calendário'   },
    retrab:  { icon:'ti-rotate-clockwise',label:'Retrabalho'   },
    negoc:   { icon:'ti-trending-up',     label:'Pipeline'     },
    ...
    users:   { icon:'ti-users',           label:'Usuários'     },
    config:  { icon:'ti-settings-2',      label:'Configurações'},
  };
...
function buildNav() {
  ...
  sb.innerHTML =
    `<button class="sb-ham" onclick="toggleSidebar()" title="Recolher menu"><i class="ti ti-menu-2"></i><span class="sb-ham-label">EYE<small>by MINT</small></span></button>` +
