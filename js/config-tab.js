...
async function wppConectar() {
  ...
  /* Embedded Signup — abre o fluxo Meta para o cliente conectar o próprio número.
     Implementação completa pendente de aprovação como Provedor de Tecnologia Meta.
     Por enquanto mostra instruções para configuração manual via suporte. */
  const el = document.getElementById('cfg-wpp-content');
  if (el) el.innerHTML = `
    <div style="padding:16px;background:var(--bg2);border-radius:12px;border:1px solid var(--bdr)">
      <p style="font-size:13px;font-weight:600;margin:0 0 8px;color:var(--txt1)">
        <i class="ti ti-info-circle" style="color:var(--ind)"></i> Conectar WhatsApp
      </p>
      <p style="font-size:12px;color:var(--txt2);margin:0 0 12px;line-height:1.6">
        Para conectar o número da sua loja, entre em contato com o suporte Eye.<br>
        A integração via Embedded Signup estará disponível em breve.
      </p>
      <button class="btn-s" onclick="_loadWppStatus()" style="font-size:12px">
        <i class="ti ti-refresh"></i> Verificar novamente
      </button>
    </div>`;
}
